...
function  init_horbar() {
			...
			if ($('#echt').length ){ 
			  ...
			  var echartBar = echarts.init(document.getElementById('echt'), theme);
			  ...
			  echartBar.setOption({
				...
				legend: {
				  ...
				  data: [
				  <?php 
		$query = "SELECT DISTINCT leave_type from durationv";
...
while ($row = mysqli_fetch_assoc($result)){ ?>
	{
	  ...
	  type: 'bar',
	  data: [<?php echo $row['Jan'] . ',' . $row['Feb'] . ',' . $row['Mar'] . ',' . $row['Apr'] . ',' . $row['May'] . ',' . $row['Jun'] . ',' . $row['Jul'] . ',' . $row['Aug'] . ',' . $row['Sep'] . ',' . $row['Oct'] . ',' . $row['Nov'] . ',' . $row['Dece'] ?>]
	},
<?php } ?>]
			  });
