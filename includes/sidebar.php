
            <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
              <div class="menu_section">
                <h3>General</h3>
                <ul class="nav side-menu">
                  <li><a href="index.php?p=home"><i class="fa fa-home"></i> Home</a></li>
                  <li><a><i class="fa fa-edit"></i> Manage <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
		      <li><a href="index.php?p=themes">Manage Theme </a>
                      <li><a href="index.php?p=activities">Activities</a></li>
                      <li><a href="index.php?p=user">Manage Staff</a></li>
<!--                       <li><a href="index.php?p=Baseline">new activities</a></li>
 -->                      <li><a href="index.php?p=adminunits">Manage Implementing Units</a></li>
                     <!--  <li><a href="index.php?p=secretariat">Secretariat</a></li> -->
		     <!--  <li><a href="index.php?p=change_pjs">update activities</a></li>		 -->				
                      <!--li><a href="form_advanced.html">Performance Measure</a></li>
                      <li><a href="form_validation.html">Form Validation</a></li>
                      <li><a href="form_wizards.html">Form Wizard</a></li>
                      <li><a href="form_upload.html">Form Upload</a></li>
                      <li><a href="form_buttons.html">Form Buttons</a></li-->
                    </ul>
                  </li>

		   <li><a><i class="fa fa-institution"></i> Performance Measures <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="index.php?p=baseline">Baseline</a></li>
                      <li><a href="index.php?p=targets">Targets</a></li>
                      <!--li><a href="index.php?p=request_leave">Request Leave</a></li-->

          
                    </ul>
                  </li>

                  <li><a><i class="fa fa-desktop"></i> Targets <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
		      <li><a href="index.php?p=schedule">Targets vs Achieved</a></li>
                      <li><a href="index.php?p=activities not started">Activities not started</a></li>
                      <!--li><a href="index.php?p="></a>Activities</li -->
					 
                    </ul>
                  </li>
		<li><a><i class="fa fa-pie-chart"></i> Reports <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="index.php?p=achieved">Achieved Targets</a></li>
		      <li><a href="index.php?p=activities">Theme Activities</a></li>
		      <li><a href="index.php?p=targets">Set Targets</a></li>                      
		      <li><a href="index.php?p=targets">Topic Completion Rate - Annually</a></li>

                  <!--li><a><i class="fa fa-table"></i> Tables <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="tables.html">Tables</a></li>
                      <li><a href="tables_dynamic.html">Table Dynamic</a></li>
                    </ul>
                  </li>
                  <li><a><i class="fa fa-bar-chart-o"></i> Data Presentation <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="chartjs.html">Chart JS</a></li>
                      <li><a href="chartjs2.html">Chart JS2</a></li>
                      <li><a href="morisjs.html">Moris JS</a></li>
                      <li><a href="echarts.html">ECharts</a></li>
                      <li><a href="other_charts.html">Other Charts</a></li>
                    </ul>
                  </li>
                  <li><a><i class="fa fa-clone"></i>Layouts <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="fixed_sidebar.html">Fixed Sidebar</a></li>
                      <li><a href="fixed_footer.html">Fixed Footer</a></li-->
                    </ul>
                  </li>
                </ul>
              </div>
              <div class="menu_section">
                <h3>Live On</h3>
                <ul class="nav side-menu">
                  <!--li><a><i class="fa fa-bug"></i> Additional Pages <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="e_commerce.html">E-commerce</a></li>
                      <li><a href="projects.html">Projects</a></li>
                      <li><a href="project_detail.html">Project Detail</a></li>
                      <li><a href="contacts.html">Contacts</a></li>
                      <li><a href="profile.html">Profile</a></li>
                    </ul>
                  </li>
                  <li><a><i class="fa fa-windows"></i> Extras <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                      <li><a href="page_403.html">403 Error</a></li>
                      <li><a href="page_404.html">404 Error</a></li>
                      <li><a href="page_500.html">500 Error</a></li>
                      <li><a href="plain_page.html">Plain Page</a></li>
                      <li><a href="login.html">Login Page</a></li>
                      <li><a href="pricing_tables.html">Pricing Tables</a></li>
                    </ul>
                  </li>
                  <li><a><i class="fa fa-sitemap"></i> Multilevel Menu <span class="fa fa-chevron-down"></span></a>
                    <ul class="nav child_menu">
                        <li><a href="#level1_1">Level One</a>
                        <li><a>Level One<span class="fa fa-chevron-down"></span></a>
                          <ul class="nav child_menu">
                            <li class="sub_menu"><a href="level2.html">Level Two</a>
                            </li>
                            <li><a href="#level2_1">Level Two</a>
                            </li>
                            <li><a href="#level2_2">Level Two</a>
                            </li>
                          </ul>
                        </li>
                        <li><a href="#level1_2">Level One</a>
                        </li>
                    </ul>
                  </li-->                  
                  <li><a href="login.php?l=logout"><i class="fa fa-laptop"></i> Log out <span class="label label-success pull-right"><?php echo $_SESSION['leave_fullname']; ?></span></a></li>
                </ul>
              </div>
            </div>
			
